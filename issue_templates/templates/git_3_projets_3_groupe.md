---
name: Multi projets à 3 - Demande client
...
Tout d'abord, vous aller récupérer différents projets (un site web de restaurant, un site web de réservation de voyages et un site web de salle de spectacle) disponibles dans un dépôt distant sur GitHub/GitLab avec seulement une branche distante master (ou main) ; pour faire ça, suivez les instructions.

## Instructions

Vous êtes dans un cas de figure où vous travaillez à trois : apprenant A, apprenant B et apprenant C

### Etape 1
**Cloner le dépôt sur sa session/son ordinateur** :
- Dans votre page d'accueil sur GitHub/GitLab, vous avez trois  nouveaux projets disponibles, intitulés "Mon Restoto", "Horizons" et "Salle Lumière": vous allez faire la même chose pour les trois projets :
...
- A partir de maintenant, **ON COMMUNIQUE !!!** : pour chaque projet, vous allez effectuer les instructions suivantes :
    - Commencer par un premier projet (par exemple "Horizons") : l'un de vous trois va créer une branche locale appelée "dev", puis la pousser sur GitHub/GitLab ; pour faire ça, l'apprenant B (par exemple) doit se placer dans le terminal sur le projet (`cd horizons`, `cd mon-restoto` ou `cd salle-lumiere`), puis taper : `git checkout -b dev`, puis `git push -u origin dev`
    ...
    - Les deux autres personnes qui n'ont pas fait la manip (les apprenants A et C par exemple) doivent maintenant récupérer la branche distante "dev", et la mettre en locale sur leur ordinateur à eux ; une fois le projet cloné, il faut se déplacer dessus dans le terminal (`cd horizons` ou `cd mon-restoto` ou cd `salle-lumiere`), puis taper : `git fetch origin`, puis `git checkout -b dev origin/dev`.
    - Les apprenants A et C viennent de venir chercher toutes les branches et les commits distants de GitHub/GitLab pour les placer en attente sur leur ordinateur, puis ont créé une branche locale appelée "dev" à partir de la branche distante d'origine "dev".
    - Vous avez donc tous les trois des branches locales "master" (ou "main") et "dev", qui pointent vers les mêmes branches distantes sur GitHub/GitLab.
- Pour le deuxième et le troisième projet, vous pouvez inverser les rôles (comme ça, chacun fait la manip une fois) : si l'apprenant B s'est occupé de créer et pousser la branche "dev" pour "Horizons", alors l'apprenant A peut le faire pour "Mon Restoto", et l'apprenant C "Salle Lumière".
...
- Maintenant, chacun travaille de son côté : l'apprenant A va travailler sur le projet "Mon Restoto", l'apprenant B va travailler sur le projet "Horizons", et l'apprenant C va travailler sur le projet "Salle Lumière"
...
- Chacun fait ça pour son projet, en mettant une autre personne en "Reviewer" :
    ...
    - l'apprenant C demande à l'apprenant A de faire la peer-review de "Salle Lumière".
...
- Maintenant, **ON INVERSE** : 
    - l'apprenant A doit passer sur le projet "Salle Lumière",
...
- Vous vous en doutez, **ON INVERSE ENCORE** :
    - l'apprenant A passe cette fois sur le projet "Horizons",
    - l'apprenant B passe cette fois sur le projet "Salle Lumière",
...
- Sur GitHub/GitLab, faites une Pull Request/Merge Request de la branche distante nouvellement poussée vers la branche distante "dev" en demandant à votre collègue d'être le "Reviewer" :
    - l'apprenant A demande à l'apprenant B de faire la peer-review de "Horizons",
    - l'apprenant B demande à l'apprenant C de faire la peer-review de "Salle Lumière",
...
- C'est terminé, les trois projets ont été modifiés, prêts à être envoyés au client. Félicitations !
