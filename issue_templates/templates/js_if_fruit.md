...
Via l'utilisation du `if`, nous allons donc exécuter une instruction si la condition est remplie (*exemple:* je définis ma condition, qui est la suivante : si le mot tapé dans la barre de recherche d'un utilisateur est `bleu`, alors mon instruction sera de donner un `background-color` bleu au body de ma page).
...
Télécharger le dossier zippé de cet exercice ici : {{ codephenix_url }}/cours/exercices/exo-js-fruit.zip
...
>*exemple* : si la réponse inclus le mot "pomme", alors non seulement nous allons afficher du texte personnalisé pour cette réponse, mais nous allons aussi ajouter la classe "pomme" à mon document.body
